...
%   The simulated output is stored in y.
    ...
    onePeriod = [ones(1, 15), zeros(1, 30), ones(1, 15)];
    numCycles = 4;
    u = repmat(onePeriod, [1, numCycles])';
    N = length(u);
    t_u = linspace(0,N/2,N);
    y = dlsim(a,b,c,d,u);
...
%   
    ...
    if verbose
        ...
        subplot(212);
        plot(t_u(1:2:end),y(1:2:end,1),'s','Linewidth', 1.2); hold on; 
