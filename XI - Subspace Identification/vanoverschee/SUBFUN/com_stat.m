...
%   Optional:
%
%           [A,B,C,D,K,R,AUX,ss] = com_stat(y,u,i,n,AUX,W,sil);
%   
...
%   Example:
%   
%           [A,B,C,D,K,R,AUX] = com_stat(y,u,10,2);
%           for k=3:6
%              [A,B,C,D] = com_stat(y,u,10,k,AUX);
%           end
%           
%   Reference:
%   
...
%           Kluwer Academic Publishers, 1996, Page 124 (Fig 4.7)
%           
%   Copyright:
%   
...
function [A,B,C,D,K,Ro,AUX,ss] = com_stat(y,u,i,n,AUXin,W,sil);

if (nargin < 7);sil = 0;end

mydisp(sil,' ');
mydisp(sil,'   Combined algorithm 2');
mydisp(sil,'   --------------------');

% Check the arguments
if (nargin < 3);error('com_stat needs at least three arguments');end
if (nargin < 4);n = [];end
if (nargin < 5);AUXin = [];end
if (nargin < 6);W = [];end
if (isempty(W));W = 'N4SID';end
